...
# Version 1.2 - dash length now has significance - single-dash options are
#   always assumed to be groups of single-letter options (e.g. tar -cvfz) and
#   double-dash always assumed to be full words.  A flag to turn this off is
#   provided in $GODashLengthSignificant (defaults to true).

$GODashLengthSignificant = TRUE;
$GODebug = FALSE;
...
function GOProcessArgument($opt, $args, $i, $numargs, $arg_lookup) {
	global $GODebug;
    if ($GODebug) {
...
function GetOptions($argDescriptions) {
	global $GODebug;
	global $GODashLengthSignificant;
	...
	while ($i < $numargs) {
		...
		if ($arg == '--') {
	        ...
	        # We're sharing a lot of state between the two - is this fragile?
			if (array_key_exists($opt,$arg_lookup)) {
		        GOProcessArgument($opt, &$args, &$i, &$numargs, &$arg_lookup);
			} else {
